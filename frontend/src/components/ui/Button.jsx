...
export const Button = ({ children, variant = 'primary', className = '', ...props }) => {
  ...
  const variants = {
    primary: isDark
      ...
      : 'bg-primary text-dark-bg hover:shadow-lg border border-primary',
    secondary: isDark
      ? 'bg-transparent text-primary border border-primary hover:bg-primary/10 hover:shadow-[0_0_20px_rgba(125,249,255,0.4)]'
      : 'bg-transparent text-primary border border-primary hover:bg-primary/5 hover:shadow-lg',
    outline: isDark
      ? 'bg-transparent text-base border border-base/30 hover:border-primary hover:text-primary'
      : 'bg-transparent text-base border border-base/20 hover:border-primary hover:text-primary',
    ghost: isDark
